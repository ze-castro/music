  ...
  const shown = $derived.by(() => {
    ...
    switch (sort) {
      ...
      case 'za':
        ...
        break;
      case 'played':
        list.sort((a, b) => (b.playCount ?? 0) - (a.playCount ?? 0) || byTitle(a, b));
        break;
      default:
...
<div class="p-4 md:p-6">
  ...
  {:else}
    ...
    <button
      ...
      style="bottom: calc(max(6px, min(env(safe-area-inset-bottom), 14px)) + {player.current ? 8.5 : 3.75}rem)"
    >
      <Shuffle size={22} />
    </button>
    {#if visible < shown.length}
      <button
        onclick={() => (visible += CHUNK)}
        class="mt-4 w-full rounded-md border py-2 text-sm hover:bg-accent"
        >Show more ({shown.length - visible} left)</button
      >
    {/if}
  {/if}
</div>
